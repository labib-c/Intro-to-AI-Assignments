...
def sudokuCSP(initial_sudoku_board, model='neq'):
    ...
    for row in var_array:
        if model == 'neq':
            ...
            constraint_list.append(AllDiffConstraint(str(var_array.index(row)), row))
    ...
    for colj in range(len(var_array[0])):
        ...
        if model == 'neq':
            ...
            constraint_list.append(AllDiffConstraint(str(colj), scope))
    ...
    for i in [0, 3, 6]:
        for j in [0, 3, 6]:
            ...
            if model == 'neq':
                ...
                constraint_list.append(AllDiffConstraint("( " + str(i) + ', ' + str(j) + " )",
                                                         scope))
...
def solve_planes(planes_problem, algo, allsolns,
                 variableHeuristic='mrv', silent=False, trace=True):
    ...
    for plane in planes:
        for i in range(max_flights):
            if i == 0:
                # Constraint 2
                var = Variable(plane + ' flight' + str(i), planes_problem.can_start(plane) + [''])
            else:
                # Constraint 1
                var = Variable(plane + ' flight' + str(i), planes_problem.can_fly(plane) + [''])
    ...
    # Constraint 3
    can_follow_lists = [list(c) for c in can_follow] + [[f, ''] for f in flights] + [['', '']]
    for i in range(len(flights_of_planes)):
        for j in range(1, len(flights_of_planes[i])):
            constraints_list.append(TableConstraint("Can follow ",
    ...
    for i in range(len(flights_of_planes)):
        for j in range(len(flights_of_planes[i]) - min_maintenance + 1):
            constraints_list.append(NValuesConstraint("Maintenance",
    ...
    constraints_list.append(AllDiffConstraint("All Flights", non_empty_variables))
    constraints_list.append(NValuesConstraint("Every flight flown", non_empty_variables,
                                              flights, len(flights), len(flights)))
        ...
        for i in range(len(planes)):
            for j in range(max_flights):
                ...
                for k in range(len(s)):
                    if s[k][0].name().split()[0] == planes[i] and s[k][1] != '':
